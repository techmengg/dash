...
/// <summary>
/// Trigger at the battle arena entrance. When the player enters from the spawn hallway,
/// shows a Souls-like area title popup.
/// </summary>
public class BattleArenaTrigger : MonoBehaviour
{
    ...
    private void TryStartCutscene(Collider2D other)
    {
        ...
        // Prevent legacy overlap: if a secondary boss trigger exists, disable it once this flow starts.
        BossArenaTrigger legacyTrigger = FindFirstObjectByType<BossArenaTrigger>();
        if (legacyTrigger != null)
            legacyTrigger.gameObject.SetActive(false);

        StartBossEncounterInBattleArena();

        // Safety cleanup: remove any legacy WaveManager that may have been created in older builds.
    ...
    private void StartBossEncounterInBattleArena()
    {
        BossEnemy boss = FindFirstObjectByType<BossEnemy>();
        if (boss == null)
            return;

        ConfigureBossArenaBoundsAtCurrentPosition(boss);
        SyncBossRigidbodyToCurrentPosition(boss);

        ApplyBossRoomZoom();

        StartCoroutine(PlayBossIntroCutscene(boss));
    }

    private void ConfigureBossArenaBoundsAtCurrentPosition(BossEnemy boss)
    {
        if (boss == null)
            return;
        ...
        {
            // Fallback for scenes without a runtime builder.
    ...
    private static void SyncBossRigidbodyToCurrentPosition(BossEnemy boss)
    {
        if (boss == null)
            return;
    ...
    private IEnumerator PlayBossIntroCutscene(BossEnemy boss)
    {
    ...
    public void ResetTrigger()
    {
        ...
        // Reset the popup so it can show again
        AreaTitlePopup popup = FindFirstObjectByType<AreaTitlePopup>();
        if (popup != null)
            Destroy(popup.gameObject);

        // Remove any legacy WaveManager created by older boss arena flow.
        WaveManager wm = FindFirstObjectByType<WaveManager>();
        if (wm != null)
            Destroy(wm.gameObject);
    }
